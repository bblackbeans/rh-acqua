"""
Views administrativas para o sistema de email integradas ao Django Admin
"""
...
def email_dashboard_admin(request):
    """
    Dashboard do sistema de email integrado ao admin
    """
    # Estatísticas gerais
    stats = {
    ...
    context = {
        'title': 'Dashboard - Sistema de Email',
        'stats': stats,
        'recent_emails': recent_emails,
        'recent_logs': recent_logs,
        'opts': EmailQueue._meta,  # Para compatibilidade com admin
    }
    
    return render(request, 'admin/email_system/dashboard.html', context)


@staff_member_required
def email_queue_admin(request):
    """
    Visualização da fila de emails no admin
    """
    ...
    context = {
        ...
        'opts': EmailQueue._meta,
    }
    
    return render(request, 'admin/email_system/queue.html', context)


@staff_member_required
def email_logs_admin(request):
    """
    Visualização dos logs de email no admin
    """
    status_filter = request.GET.get('status', '')
    ...
    return render(request, 'admin/email_system/logs.html', context)


@staff_member_required
@require_http_methods(["POST"])
def retry_email_admin(request, email_id):
    """
    Tenta reenviar um email que falhou (admin)
    """
    try:
        ...
        if not email_queue.can_retry():
            ...
            return redirect('admin:email_system_queue')
...
@staff_member_required
@require_http_methods(["POST"])
def cancel_email_admin(request, email_id):
    """
    Cancela um email pendente (admin)
    """
    try:
        email_queue = EmailQueue.objects.get(id=email_id)
...
class TestEmailAdminView(View):
    """
    View para testar envio de emails (admin)
    """
...
@staff_member_required
def email_stats_api_admin(request):
    """
    API para estatísticas do sistema de email (admin)
    """
    stats = {
        'total_emails': EmailQueue.objects.count(),
        'pending_emails': EmailQueue.objects.filter(status='pending').count(),
